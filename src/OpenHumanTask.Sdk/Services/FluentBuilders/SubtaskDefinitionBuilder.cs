...
namespace OpenHumanTask.Sdk.Services.FluentBuilders
{
        ...
        : ISubtaskDefinitionBuilder
    {
        ...
        public virtual ISubtaskDefinitionBuilder WithDefinition(string definitionId)
        {
            ...
            if (!definitionId.IsTaskDefinitionReference()) throw new ArgumentException(nameof(definitionId), new FormatException($"The specified value '{definitionId}' is not a valid human task definition reference."));
        ...
        {
            if (input != null && input is string expression && !expression.IsRuntimeExpression())
                throw new ArgumentNullException(nameof(input), $"The specified value '{expression}' is not a valid runtime expression, or does not use the mandatory '${{ expression }}' format");
